...
const AddStlcActivities = (props) => {
  ...
  const [standardBreakup, setStandardBreakup] = useState("");
  ...
  const handleSetStandardBreakup = (event) => {
    setStandardBreakup(event.target.value);
  };
  ...
  function handleSubmit() {
    ...
    // console.log("stlcName",stlcName)
    // console.log("standardBreakup",standardBreakup)
    ...
    // return false
    // return false
    // alert(stlcPhase+'--'+stlcName+'--'+standardBreakup);
    let body = {
      // stlc: stlc,
      stlcPhase: stlcPhase,
      stlcName: stlcName,
    ...
    if (
      ...
      promise.then((values) => {
        if (values.status === 200) {
          swal("Success!", "STLC Activity Added Successfully!", "success").then(
          ...
          <div sx={{ gap: "10px" }}>
            <FormControl>
              ...
              <Select
                ...
                label="STLC Phase"
              >
                {stlcPhases.map((category, index) => (
                  <MenuItem key={index} value={category.phase_id}>
                    {category.phase_name}
                  </MenuItem>
                ))}
              </Select>             
            ...
            <TextField
              ...
              label="STLC Activity Name"
            />
            <br /> <br />
            <TextField
              style={{ width: 400 }}
              id="standard-basic"
              value={standardBreakup}
              onChange={handleSetStandardBreakup}
              label="Benchmark Effort"
            />
            <br /> <br />
          </div>
        </DialogContent>
...
export default AddStlcActivities;
